"""Shared style and helpers for the manuscript plates (version 10) in docs/study.

Adapted copy of `technical/sources/v10_figstyle.py` of revision 10: it reads the public data of
`docs/study/data/` and writes the plates to `docs/study/figures/<language>/` (PNG at 300 dpi, SVG and PDF)
with a preview and a QA record in `docs/study/qa/`.

Figure rules follow the target journal's August 2026 Information for Authors
(recorded in study/journal_guidelines.md): lowercase panel letters, no titles inside the artwork, no box
...
BASE = Path(__file__).resolve().parent          # docs/study
assert BASE.name == 'study' and BASE.parent.name == 'docs', BASE
