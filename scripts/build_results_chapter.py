...
`tests/test_page_figure_code.py` fails if the code in the page stops matching the module.
    ...
    python scripts/build_results_chapter.py            # rewrite the page
    python scripts/build_results_chapter.py --check    # exit 1 if the page is out of date
...
HEADER = '''---
title: "Results"
subtitle: "The principal findings, each drawn by the code shown with it"
---
'''
...
```{{python}}
#| label: preparacion
# Everything the figures below need. This chapter is generated from docs/study/figuras_*.py by
# scripts/build_results_chapter.py, so the code printed under each figure is the code that draws it;
# tests/test_page_figure_code.py fails if the two ever drift apart.
import sys
from pathlib import Path
...
#| label: resumen
kpis([
    ("13", "Figures drawn by this chapter from the published tables"),
    (str(len(rh.data_tables())), "Tables the chapter reads"),
    ("2019–2025", "Observation window"),
    ("F84 without F84.2", "Case definition of the main analysis"),
])
```

These are the principal results of the study: four panel figures that carry the main findings, nine
supplementary figures that support them, and the tables they are built from. Every one is drawn here,
at render time, by the code printed with it — nothing on this page is a stored image.

Case definition: hospital episodes with an eligible F84 code, excluding Rett syndrome (F84.2); community
programmes with the strict autism code except where the "F84 family" is indicated. Every count is
**administrative recognition, not prevalence or incidence**: a count of episodes, programme entries,
follow-up stocks or school enrolments, each with its unit, its denominator and its coverage.

::: {.callout-note}
## Two figures are redraws rather than original artwork

The data-flow schematic and the territorial maps were static images for most of the study's history;
no code in this repository drew them. The code shown with them was written from the published
aggregates and reproduces the same panels and the same numbers, but the artwork is not pixel-identical
to the original. The territorial maps in particular use an equal-area projection fitted to the printed
original, which drew the shapefile unprojected, so their aspect ratio differs by one to two per cent.
:::
...
def main():
    ...
    if args.check:
        ...
        if before != after:
            ...
            raise SystemExit("docs/version_10_panels.qmd is out of date: run python scripts/build_results_chapter.py")
