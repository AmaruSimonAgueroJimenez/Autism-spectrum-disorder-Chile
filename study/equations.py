# -*- coding: utf-8 -*-
"""equations.py — todas las fórmulas del estudio multisistema.
...
Uso:  python3 study/equations.py     (compone los PNG y lista las rutas)
"""
...
ESTIMATORS: list[dict] = [
    dict(key="crude_rate", eq=["crude"],
         ...
         impl="scripts/epi_helpers.py::crude_rate / poisson_limits; study/common.py::rate_per",
         script="study/pipeline/01_grd_core.py, 01b_deis_egresos.py, 02_rem_pathway.py, 11_grd_episode_detail.py",
         output="study/outputs/tidy/grd_year_summary.csv"),
    dict(key="age_specific", eq=["age_specific"],
         ...
         script="study/pipeline/03_denominators.py, 06_models.py",
         output="study/outputs/tidy/models_population_rates.csv"),
    dict(key="direct_standardisation", eq=["dsr", "dsr_var", "ff"],
         ...
         script="study/pipeline/06_models.py::_standardise, population_rates",
         output="study/outputs/tidy/models_population_rates.csv"),
    dict(key="rate_ratio", eq=["rr"],
         ...
         script="study/pipeline/06_models.py::population_rates",
         output="study/outputs/tidy/models_population_rates.csv"),
    dict(key="count_ratio", eq=["count_ratio"],
         ...
         script={"es": "study/pipeline/14_extra_figures_rem.py::prep_e14 (razón hombres:mujeres por grupo etario, A05)",
                 "en": "study/pipeline/14_extra_figures_rem.py::prep_e14 (male:female ratio by age group, A05)"},
         output="study/outputs/<variante>/<idioma>/extra/tables/E14_rem_a05_age_sex_numeric.csv"),
    dict(key="indirect_standardisation", eq=["expected", "byar"],
         ...
         impl="study/pipeline/15b_spatial_correlation.py::indirect_standardise, national_age_sex_schedule; "
              "scripts/epi_helpers.py::expected_counts, standardized_ratio",
         script="study/pipeline/15b_spatial_correlation.py::indirect_standardise",
         output="study/outputs/tidy/spatial_comuna_standardised.csv; spatial_region_summary.csv; "
    ...
    dict(key="empirical_bayes", eq=["eb", "eb_prior"],
         ...
         impl="study/pipeline/15b_spatial_correlation.py::indirect_standardise (Marshall 1991: sir_eb, "
    ...
    dict(key="quasi_poisson", eq=["qpois", "apc"],
         ...
         impl="study/common.py::quasi_poisson_trend; scripts/epi_helpers.py::annual_percent_change",
         script="study/pipeline/06_models.py::fit_glm, Store.add_trend",
         output="study/outputs/tidy/models_summary.csv"),
    dict(key="hospital_fixed_effects", eq=["fe"],
         ...
         script="study/pipeline/06_models.py::hospital_models",
         output="study/outputs/tidy/hospital_effects.csv"),
    dict(key="random_intercept", eq=["ri"],
         ...
         script="study/pipeline/06_models.py::fit_random_intercept",
         output="study/outputs/tidy/hospital_effects.csv"),
    dict(key="durbin_watson", eq=["dw"],
         ...
         impl="study/pipeline/06_models.py::durbin_watson",
         script="study/pipeline/06_models.py",
         output="study/outputs/tidy/models_summary.csv"),
    dict(key="wilson", eq=["wilson"],
         ...
         impl="study/common.py::wilson",
         script="study/pipeline/11_grd_episode_detail.py",
         output="study/outputs/tidy/grd_readmission.csv"),
    dict(key="jeffreys", eq=["jeffreys"],
         ...
         impl="study/pipeline/13_extra_figures_hospital.py::jeffreys (scipy.stats.beta.ppf)",
         script={"es": "study/pipeline/13_extra_figures_hospital.py::ef4 (letalidad hospitalaria por año y posición)",
                 "en": "study/pipeline/13_extra_figures_hospital.py::ef4 (in-hospital lethality by year and position)"},
         output="study/outputs/<variante>/<idioma>/extra/tables/EF4_severity_weight_numeric.csv"),
    dict(key="seasonal_index", eq=["season"],
         ...
         impl="scripts/epi_helpers.py::seasonal_index; study/pipeline/11_grd_episode_detail.py",
         script="study/pipeline/11_grd_episode_detail.py, 08b_figures_rem.py",
         output="study/outputs/tidy/grd_monthly.csv"),
    dict(key="index_numbers", eq=["index_number"],
         ...
         impl="study/pipeline/06_models.py::convergence_index",
         script="study/pipeline/06_models.py",
         output="study/outputs/tidy/models_convergence_index.csv"),
    dict(key="spearman", eq=["spearman"],
         ...
         impl="scipy.stats.spearmanr; np.arctanh; study/pipeline/15b_spatial_correlation.py::spearman_ci",
         script="study/pipeline/06_models.py::figure_hospital; 15b_spatial_correlation.py::spearman_ci",
         output="study/outputs/tidy/hospital_effects.csv; spatial_correlations.csv; spatial_stability.csv"),
    ...
    dict(key="survey_taylor", eq=["taylor"],
         ...
         impl="study/common.py::survey_proportion",
         script="study/pipeline/04_surveys.py::estimate",
         output="study/outputs/tidy/survey_estimates.csv"),
    dict(key="design_effect", eq=["deff"],
         ...
         impl="study/pipeline/04_surveys.py::estimate",
         script="study/pipeline/04_surveys.py",
         output="study/outputs/tidy/survey_estimates.csv"),
    dict(key="moran_global", eq=["moran"],
         ...
         script="study/pipeline/15b_spatial_correlation.py::load_geography, moran_global",
         output="study/outputs/tidy/spatial_moran.csv"),
    dict(key="moran_bivariate", eq=["moran_biv"],
         ...
         script="study/pipeline/15b_spatial_correlation.py::moran_bivariate",
         output="study/outputs/tidy/spatial_bivariate_moran.csv; spatial_bivariate_local.csv"),
    dict(key="lisa", eq=["lisa", "bh"],
         ...
         script="study/pipeline/15b_spatial_correlation.py::lisa_local, bh_threshold",
         output="study/outputs/tidy/spatial_lisa.csv"),
    dict(key="getis_ord", eq=["gistar", "bh"],
         ...
         impl="esda.getisord.G_Local(star=True, permutations=999, seed=SEED); "
              "statsmodels.stats.multitest.multipletests(method='fdr_bh', alpha=0.05)",
         script="study/pipeline/15b_spatial_correlation.py::gistar_local, bh_threshold",
         output="study/outputs/tidy/spatial_lisa.csv"),
    dict(key="lorenz_gini", eq=["lorenz", "gini"],
         ...
         impl="study/pipeline/15b_spatial_correlation.py::lorenz_gini (trapecios sobre la curva acumulada)",
         script="study/pipeline/15b_spatial_correlation.py::lorenz_gini, decile_ratio",
         output="study/outputs/tidy/spatial_inequality.csv"),
    dict(key="theil", eq=["theil"],
         ...
         impl="study/pipeline/15b_spatial_correlation.py::theil_decomposition (descomposición intra/entre regiones)",
         script="study/pipeline/15b_spatial_correlation.py::theil_decomposition",
         output="study/outputs/tidy/spatial_inequality.csv"),
    dict(key="suppression", eq=["suppression"],
         ...
         impl="study/pipeline/11_grd_episode_detail.py (suppression_flag, n_episodes_display); "
              "15b_spatial_correlation.py (columna suppressed de las tablas comunales)",
         script="study/pipeline/11_grd_episode_detail.py, 01b_deis_egresos.py, 15b_spatial_correlation.py",
         output="study/outputs/tidy/grd_territory.csv; spatial_comuna_indicators.csv"),
]
