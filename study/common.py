# -*- coding: utf-8 -*-
"""common.py — utilidades compartidas del pipeline del estudio: escritura atómica, hashes, lectura por trozos,
...
def atomic_write_csv(df: pd.DataFrame, path: Path, **kwargs) -> Path:
    ...
    # Modo revista (PLATE_JOURNAL): el corpus no recibe escrituras; la identidad cuando está apagado.
...
# ===========================================================================
# Modo revista de láminas (PLATE_JOURNAL)
# ---------------------------------------------------------------------------
...
#
# Encendido (`PLATE_JOURNAL=1`, o el nombre de la variante que se envía; por omisión `sin_rett`),
# la MISMA lámina se guarda en la forma que pide la revista:
...
# ===========================================================================
PLATE_JOURNAL_ENV = "PLATE_JOURNAL"
...
def plate_journal_mode() -> bool:
    """¿Está encendido el modo revista? Solo lo enciende la variable de entorno PLATE_JOURNAL."""
...
def journal_scratch_dir() -> Path:
    ...
    return Path(tempfile.gettempdir()) / "study_journal_scratch"
...
def journal_plate_export(fig, path, lang: str | None = None) -> Path:
    ...
    verificador de composición sobre el estado que se guarda (PLATE_CHECK), anota los títulos
    ...
    lang = (lang or path_lang or getattr(fig, "_plate_lang", None) or getattr(fig, "_study_lang", None)
            or "es")
...
def save_fig(fig, path: Path, dpi=600):
    ...
    plate_pct_bind(fig)
    # Modo revista (PLATE_JOURNAL): títulos fuera, convención numérica, verificación y ruta de la
    ...
    # comprobarse cuando PLATE_CHECK está en «report» o en «strict».
...
def plate_resolve(fig) -> None:
    ...
    # Modo revista (PLATE_JOURNAL, apagado por omisión): los títulos de panel se reducen a la letra
    ...
    # `plate_resolve` justo antes de guardar), de modo que encender PLATE_CHECK comprueba la
...
# ---------------------------------------------------------------------------
#: Modo del verificador automático. Se lee de la variable de entorno PLATE_CHECK
...
def plate_check_enable(mode: str = "strict", **kw) -> None:
    ...
    Equivale a exportar PLATE_CHECK=strict|report antes de ejecutar el módulo."""
    mode = str(mode).lower()
