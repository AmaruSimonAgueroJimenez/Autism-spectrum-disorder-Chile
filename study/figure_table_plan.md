...
`study/supplementary_material.py` (`FIGURE_ORDER`, `TABLE_ORDER`, `FIGURE_GROUPS`, `TABLE_GROUPS`); la lista
...
* **Composición medida, no supuesta.** `common.check_layout(fig)` mide la lámina terminada sobre su renderizador real y
  ...
  embudos que todos atraviesan (`plate_resolve` y `save_fig`), con `PLATE_CHECK=off|report|strict`. Los defectos se
...
* La **composición dentro del panel** dejó de confiarse a la vista: `common.check_layout` la mide sobre el renderizador
  real y los nueve módulos la ejecutan con `PLATE_CHECK`. Los defectos que las verificaciones nombraron —títulos
