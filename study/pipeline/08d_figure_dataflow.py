...
Ejecución: `python3 study/pipeline/08d_figure_dataflow.py` desde la raíz del repositorio.
...
SCRIPT = "study/pipeline/08d_figure_dataflow.py"
...
def build_counts(D: SimpleNamespace, variant: str) -> Registry:
    R = Registry(variant)
    V = D.values[variant]
    VJ = f"study/outputs/values_{variant}.json"

    def T(name):
        return f"study/outputs/tidy/{name}.csv"
