...
(paso 11 del pipeline la revista de destino).
...
Uso: python3 study/pipeline/11_grd_episode_detail.py
...
def fixed_panel_codes() -> set[str]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Falta {path}: ejecute antes study/pipeline/01_grd_core.py")
...
def comuna_lookup() -> tuple[dict[str, tuple[int, int]], dict[str, str], pd.DataFrame]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Falta {path}: ejecute antes study/pipeline/03_denominators.py")
