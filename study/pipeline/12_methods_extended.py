...
manuscrito inserta, después de las Referencias, en la parte suplementaria del mismo archivo .docx.
  ...
  1. `study/equations.py` — todas las fórmulas del estudio en LaTeX (lista EQUATIONS,
  ...
  2. `study/prose_methods_extended.py` — `methods_blocks(variant, V, lang, R)` devuelve los bloques
...
Uso:
    python3 study/pipeline/12_methods_extended.py
    python3 study/pipeline/12_methods_extended.py --variants con_rett --langs en --only-missing
    python3 study/pipeline/12_methods_extended.py --keep-test-docx /ruta/de/salida
"""
...
#: «study/pipeline/ (listado en tiempo de construcción)» (Tabla S10)— se escribieron como
...
def check_blocks(ctl: Controls, variant: str, lang: str, blocks: list, bib_keys: set) -> dict:
    ...
    ctl.add("citation_keys_in_bib", f"{variant}|{lang}", 0, len(missing),
            f"claves ausentes de references.bib: {missing or 'ninguna'}")
