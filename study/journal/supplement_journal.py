...
`docx_builder.build_document`, of a single supplementary document for *the target journal*:
  ...
  Part A         the APPLIED METHODOLOGY in full, verbatim from `prose_methods_extended.methods_blocks` so that the
                 33 equations of `equations` print once each, in numeric order, under the estimator that
...
(`equations`, `prose_methods_extended.SENSITIVITY_GRID`); nothing is typed by hand. The journal number
...
def equation_problems(blocks: list) -> list[str]:
    """Every equation of `equations` printed exactly once, in numeric order, under a heading that cites it."""
