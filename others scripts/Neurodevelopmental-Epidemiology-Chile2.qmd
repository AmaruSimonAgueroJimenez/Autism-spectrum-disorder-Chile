...
ALL_NEURODEG_CODES = ALL_NEURODEV_CODES
NEURODEG_CODES_F = NEURODEV_CODES_F84
NEURODEG_CODES_G = NEURODEV_CODES_F90

# Lists for filtering - ASD codes (analogous to F-codes)
F_CODES_LIST = ['F84', 'F84.0', 'F84.1', 'F84.2', 'F84.3', 'F84.4', 'F84.5', 'F84.8', 'F84.9']
# Lists for filtering - ADHD codes (analogous to G-codes)  
G_CODES_LIST = ['F90', 'F90.0', 'F90.1', 'F90.8', 'F90.9']
# Other developmental codes
OTHER_CODES_LIST = ['F80', 'F81', 'F82', 'F83', 'F88', 'F89']
...
def is_valid_neurodeg_code(code_str):
    ...
    # F80-F83, F88-F89: Other developmental disorders
    if code_3char in ['F80', 'F81', 'F82', 'F83', 'F88', 'F89']:
...
#| fig-cap: "ASR trends for ASD codes (F84)"
#| fig-width: 16
#| fig-height: 8
...
plt.suptitle('Age-Standardized Rates - ASD codes (F84)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
#| fig-cap: "ASR trends for ADHD codes (F90)"
#| fig-width: 16
#| fig-height: 8
...
plt.suptitle('Age-Standardized Rates - ADHD codes (F90)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
#| fig-cap: "Overall neurodevelopmental disorder hospitalization trends"
#| fig-width: 10
#| fig-height: 6
...
plt.title('neurodevelopmental disorders: Hospitalizations and ASR Trends', fontweight='bold', fontsize=14)
fig.tight_layout()
...
#| tbl-cap: "Summary statistics of neurodevelopmental disorder hospitalizations"
#| tbl-cap-location: bottom
...
## Separate Analysis: ASD codes (F84) vs ADHD codes (F90)
...
#| tbl-cap: "Age-standardized rates - ASD codes (F84) - National"
#| tbl-cap-location: bottom
...
#| tbl-cap: "Mean ASR by region - ASD codes (F84)"
#| tbl-cap-location: bottom
...
#| fig-cap: "ASR trend - ASD codes (F84) by sex"
#| fig-width: 12
#| fig-height: 5
...
ax1.set_xlabel('Year')
ax1.set_ylabel('ASR (per 100,000)')
ax1.set_title('ASD codes (F84) - ASR by Sex', fontweight='bold')
ax1.legend()
ax1.grid(True, alpha=0.3)
ax1.set_xticks(YEARS)
...
#| tbl-cap: "Age-standardized rates - ADHD codes (F90) - National"
#| tbl-cap-location: bottom
...
#| tbl-cap: "Mean ASR by region - ADHD codes (F90)"
#| tbl-cap-location: bottom
...
#| fig-cap: "ASR trend - ADHD codes (F90) by sex"
#| fig-width: 14
#| fig-height: 6
...
ax1.set_xlabel('Year')
ax1.set_ylabel('ASR (per 100,000)')
ax1.set_title('ADHD codes (F90) - ASR by Sex', fontweight='bold')
ax1.legend()
ax1.grid(True, alpha=0.3)
ax1.set_xticks(YEARS)
...
#| fig-cap: "Comparison of ASD codes (F84) vs ADHD codes (F90)"
#| fig-width: 14
#| fig-height: 10
...
ax1 = axes[0, 0]
ax1.plot(asr_f['year'], asr_f['asr_total'], marker='o', linewidth=2, label='ASD codes (F84)', color='royalblue')
ax1.plot(asr_g['year'], asr_g['asr_total'], marker='s', linewidth=2, label='ADHD codes (F90)', color='seagreen')
ax1.set_xlabel('Year')
ax1.set_ylabel('ASR (per 100,000)')
...
summary_fg = pd.DataFrame({
    'Category': ['ASD codes (F84)', 'ADHD codes (F90)'],
...
#| tbl-cap: "ASD codes (F84) ASR by Comuna - Metropolitan Region"
#| tbl-cap-location: bottom
...
#| tbl-cap: "ADHD codes (F90) ASR by Comuna - Metropolitan Region"
#| tbl-cap-location: bottom
...
if len(comuna_asr_f) > 0:
    ...
    ax1.set_title('ASD codes (F84) - All Comunas', fontweight='bold')
    ax1.grid(True, alpha=0.3, axis='x')
...
if len(comuna_asr_g) > 0:
    ...
    ax2.set_title('ADHD codes (F90) - All Comunas', fontweight='bold')
    ax2.grid(True, alpha=0.3, axis='x')
...
if has_spatial_data:
    ...
    if has_f_data:
        ...
        ax1.set_title('ASD codes (F84) - ASR by Comuna', fontweight='bold', fontsize=12)
    else:
    ...
    if has_g_data:
        gdf_fg.plot(column='asr_g', cmap='Greens', linewidth=0.5, ax=ax2, edgecolor='0.5',
                    legend=True, legend_kwds={'label': 'ASR (per 100,000)', 'shrink': 0.6})
        ax2.set_title('ADHD codes (F90) - ASR by Comuna', fontweight='bold', fontsize=12)
    else:
...
if moran_f is not None:
    ...
    axes[0].set_title('ASD codes (F84)', fontweight='bold')
...
if moran_g is not None:
    ...
    axes[1].set_title('ADHD codes (F90)', fontweight='bold')
...
#| tbl-cap: "LISA Clusters - ADHD codes (F90)"
#| tbl-cap-location: bottom
...
if has_spatial_data and 'cluster_f' in gdf_fg.columns:
    ...
    ax1.set_title('LISA Clusters - ASD codes (F84)', fontsize=12, fontweight='bold')
    ax1.set_axis_off()

    ax2 = axes[1]
    gdf_fg.plot(ax=ax2, color=gdf_fg['cluster_g'].map(colors_dict), edgecolor='gray', linewidth=0.5)
    ax2.set_title('LISA Clusters - ADHD codes (F90)', fontsize=12, fontweight='bold')
    ax2.set_axis_off()
...
def get_category(code):
    if code.startswith('F84'):
        return 'Autism Spectrum Disorders (ASD)'
