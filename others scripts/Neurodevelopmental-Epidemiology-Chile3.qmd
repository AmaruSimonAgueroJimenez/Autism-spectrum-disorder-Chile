...
> **Estado: informe histórico pendiente de revisión.** Este documento conserva el análisis histórico por menciones de códigos. Para trayectorias de pacientes y posición principal/secundaria, usar la nueva extracción; sumar subtipos puede duplicar hospitalizaciones. [Informe consolidado y auditoría](consolidacion-rem-grd.html).
...
grd_data_path = os.path.join(
    os.environ.get("ASESORIAS_DATA_ROOT", "/Volumes/Datos/Asesorias_Data"),
    "GRD",
)
...
# =============================================================================
# ICD-10 CODES FOR AUTISM SPECTRUM DISORDERS (F84)
# =============================================================================
...
def is_valid_f84_code(code_str):
    """
    Validate ICD-10 codes for Autism Spectrum Disorders (F84.x only).
...
def load_and_filter_f84_data(years=YEARS, chunksize=100000):
    ...
    for year in years:
        ...
        files = glob.glob(os.path.join(grd_data_path, pattern))
        if not files:
            continue
...
rm_summary.columns = ['Comuna', 'Total Hosp.', 'ASR', 'Main ICD', 'Main Diagnosis']
...
plt.tight_layout()
save_figure(fig, 'rm_hospitalizations_f84.png')
plt.show()
```
...
#| tbl-cap: "Most prevalent diagnosis by comuna - All comunas with ASR"
#| tbl-cap-location: bottom
...
#| fig-cap: "Diagnosis ASR distribution by comuna - All comunas"
#| fig-width: 18
#| fig-height: 16
...
ax1.set_title('Diagnosis ASR by Comuna - F84 Codes', fontweight='bold')
...
ax2.set_title('Diagnosis Composition by Comuna (ASR)', fontweight='bold')
ax2.legend(title='ICD Code', bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=7)
ax2.grid(True, alpha=0.3, axis='x')

plt.tight_layout()
save_figure(fig, 'disease_by_comuna_f84.png')
plt.show()
```
...
main_icd_desc = ALL_ASD_CODES.get(main_icd_rm, 'N/A')
...
#| fig-cap: "ASR Distribution - ASD (F84) - Gran Santiago"
#| fig-width: 10
#| fig-height: 8
...
if has_spatial_data:
    ...
    if has_f84_data:
        gdf_f84.plot(column='asr_f84', cmap='Blues', linewidth=0.5, ax=ax, edgecolor='0.5',
                    legend=True, legend_kwds={'label': 'ASR (per 100,000)', 'shrink': 0.6})
        ax.set_title('Autism Spectrum Disorders (F84) - ASR by Comuna', fontweight='bold', fontsize=12)
    else:
        gdf_f84.plot(ax=ax, color='lightgrey', edgecolor='0.5', linewidth=0.5)
    ...
    plt.tight_layout()
    save_figure(fig, 'spatial_choropleth_f84.png')
    plt.show()
else:
...
plt.tight_layout()
save_figure(fig, 'moran_scatterplot_f84.png')
plt.show()
```
...
```{python}
#| label: tbl-lisa-clusters-f84
#| tbl-cap: "LISA Clusters - ASD codes (F84)"
#| tbl-cap-location: bottom
...
```{python}
#| label: fig-lisa-maps
#| fig-cap: "LISA Cluster Map - ASD codes (F84)"
#| fig-width: 10
#| fig-height: 8
...
if has_spatial_data and 'cluster_f84' in gdf_f84.columns:
    ...
    gdf_f84.plot(ax=ax, color=gdf_f84['cluster_f84'].map(colors_dict), edgecolor='gray', linewidth=0.5)
    ax.set_title('LISA Clusters - ASD codes (F84)', fontsize=12, fontweight='bold')
    ax.set_axis_off()
    ...
    plt.suptitle('Local Indicators of Spatial Association (LISA)\nGran Santiago - ASD ASR (2019-2024)', 
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    save_figure(fig, 'lisa_clusters_f84.png')
    plt.show()
else:
    fig, ax = plt.subplots(figsize=(8, 6))
...
if has_spatial_data and 'cluster_f84' in gdf_f84.columns:
    ...
    ][['Comuna', 'asr_f84', 'cluster_f84', 'lisa_p_f84']].copy()
...
#| fig-cap: "LISA Significance Map (p < 0.05)"
#| fig-width: 10
#| fig-height: 8

if has_spatial_data and 'lisa_p_f84' in gdf_f84.columns:
    fig, ax = plt.subplots(figsize=(10, 8))

    gdf_f84['sig_f84'] = gdf_f84['lisa_p_f84'] < 0.05
    gdf_f84.plot(column='sig_f84', cmap='RdYlGn_r', ax=ax, edgecolor='gray', linewidth=0.5, legend=False)
    for idx, row in gdf_f84[gdf_f84['sig_f84']].iterrows():
        centroid = row.geometry.centroid
        ax.annotate(row['Comuna'][:8], xy=(centroid.x, centroid.y), fontsize=6, ha='center', fontweight='bold')
    ax.set_title('F84 codes - Significant Clusters (p < 0.05)', fontsize=12, fontweight='bold')
    ax.set_axis_off()

    plt.suptitle('LISA Significance Map - Gran Santiago', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    save_figure(fig, 'lisa_significance_f84.png')
    plt.show()
else:
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.text(0.5, 0.5, 'Spatial data not available', ha='center', va='center', fontsize=14)
...
#| tbl-cap: "Spatial Analysis Summary - ASD (F84)"
#| tbl-cap-location: bottom
...
## Diagnosis-Specific Analysis - Metropolitan Region Comunas
...
if has_spatial_data and gdf_f84 is not None:
    for icd in icd_codes_in_rm:
...
#| tbl-cap: "Diagnosis-specific hospitalization summary - Metropolitan Region"
#| tbl-cap-location: bottom
...
#| fig-cap: "Diagnosis distribution comparison - Metropolitan Region"
#| fig-width: 16
#| fig-height: 6
...
plt.tight_layout()
save_figure(fig, 'disease_overview_rm_f84.png')
plt.show()
```
...
#| fig-cap: "All F84 Diagnoses - ASR Choropleth Comparison"
#| fig-width: 18
#| fig-height: 16
...
if has_spatial_data:
    ...
    if n_diseases > 0:
        ...
        plt.suptitle('F84 Diagnosis-Specific ASR Distribution - Metropolitan Region (2019-2024)', 
                     fontsize=14, fontweight='bold', y=1.01)
        plt.tight_layout()
        save_figure(fig, 'all_diseases_choropleth_rm_f84.png')
        plt.show()
    else:
        fig, ax = plt.subplots(figsize=(8, 6))
...
#| tbl-cap: "Spatial Autocorrelation (Moran's I) - All F84 Diagnoses"
#| tbl-cap-location: bottom
...
#| fig-cap: "Moran's I Comparison Across F84 Diagnoses"
#| fig-width: 14
#| fig-height: 6
...
if has_spatial_data and len(disease_moran) > 0:
    ...
    plt.tight_layout()
    save_figure(fig, 'moran_comparison_diseases_f84.png')
    plt.show()
else:
    fig, ax = plt.subplots(figsize=(8, 6))
...
#| tbl-cap: "Diagnosis-Specific Spatial Analysis Summary"
#| tbl-cap-location: bottom
