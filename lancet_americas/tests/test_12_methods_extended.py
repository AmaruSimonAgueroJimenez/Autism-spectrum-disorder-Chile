...
Verifican que la numeración de `equations_lancet.py` es correlativa, que cada estimador tiene ecuación y
...
REPO = LANCET.parent
for _p in (str(LANCET), str(REPO / "paper")):
...
class BlocksTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.bib = set(parse_bib(LANCET / "references_lancet.bib"))
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for (variant, lang), blocks in self.blocks.items():
                ...
                report = DB.build_document(list(blocks) + [("refs", None)], lang, out,
                                           bib_path=LANCET / "references_lancet.bib", supplementary_prefix=True)
