...
cobertura, **todos** los estimadores con su fórmula (bloques `eq` con los PNG de `equations_lancet.py` y su
...
#: Marcadores comodín de las rutas declaradas por `equations_lancet.ESTIMATORS` y por `PIPELINE_MAP`. La forma
...
def _t_sources(lang: str) -> dict:
    ...
    note = _note(lang, {"es": "una familia de archivos fuente", "en": "one source file family"}, len(rows),
                 "lancet_americas/config.py (PATHS) y outputs/tidy/data_provenance.csv",
...
def _t_case_definitions(variant: str, lang: str) -> dict:
    ...
    note = _note(lang, {"es": "un subcódigo CIE-10", "en": "one ICD-10 subcode"}, len(rows),
                 "lancet_americas/config.py (F84_SUBCODES, RETT_GRD, VARIANTS); CIE-10 (OMS) [@who_icd10]",
...
def _t_rem_codes(lang: str) -> dict:
    ...
    note = _note(lang, {"es": "un código de prestación REM", "en": "one REM service code"}, len(rows),
                 "lancet_americas/config.py y outputs/tidy/rem_code_dictionary_check.csv",
...
def _t_estimator_map(lang: str) -> dict:
    ...
    note = _note(lang, {"es": "un estimador", "en": "one estimator"}, len(rows),
                 "lancet_americas/equations_lancet.py (EQUATIONS, ESTIMATORS)",
...
def _t_sensitivity(lang: str) -> dict:
    ...
    note = _note(lang, {"es": "un análisis de sensibilidad", "en": "one sensitivity analysis"}, len(rows),
                 "lancet_americas/analysis_plan.md y outputs/tidy/models_summary.csv",
...
def _t_pipeline(lang: str) -> dict:
    ...
    for name in files:
        ...
        numeric.append(dict(script=f"lancet_americas/pipeline/{name}", documented=int(meta is not None), outputs=outputs))
    for name, meta in [("equations_lancet.py", {"what": {"es": "Define en LaTeX todas las fórmulas del estudio y compone los PNG numerados.",
    ...
    note = _note(lang, {"es": "un script del pipeline", "en": "one pipeline script"}, len(rows),
                 {"es": "lancet_americas/pipeline/ (listado en tiempo de construcción)",
                  "en": "lancet_americas/pipeline/ (listed at build time)"},
...
def _t_software(lang: str) -> dict:
    ...
    spatial = "lancet_americas/pipeline/15b_spatial_correlation.py"
    ...
    note = _note(lang, {"es": "un componente de software o constante", "en": "one software component or constant"}, len(rows),
                 {"es": "importlib.metadata en tiempo de construcción; lancet_americas/pipeline/15b_spatial_correlation.py "
                        "(SEED, PERMUTATIONS, FDR_Q, ALPHA)",
                  "en": "importlib.metadata at build time; lancet_americas/pipeline/15b_spatial_correlation.py "
                        "(SEED, PERMUTATIONS, FDR_Q, ALPHA)"},
...
# ---------------------------------------------------------------------------
# Texto: un párrafo por estimador (mismo orden y claves que equations_lancet.ESTIMATORS)
# ---------------------------------------------------------------------------
P_M4_INTRO = [
