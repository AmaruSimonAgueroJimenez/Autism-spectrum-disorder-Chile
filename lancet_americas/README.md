# Administrative recognition of autism in Chile, 2019–2025: multi-system study (The Lancet Regional Health – Americas)

Folder created for the editorial and analytical revision described in `PROMPT_CODEX_LANCET_AMERICAS.md`.
...
python lancet_americas/pipeline/run_all.py                    # runs 00 → 17 in order (10 requires soffice and pdftoppm)
python lancet_americas/pipeline/run_all.py --from 06          # resumes from a step
```
...
* **`manuscript/04_submission_lancet_americas/`**: what is submitted to *The Lancet Regional Health – Americas*:
...
`04_submission_lancet_americas` and the dated revisions `05_` to `10_revision_*`) is in `manuscript/LEEME.md`; the
...
corpus of results are in `docs/lancet/` and are read in `docs/lancet.html`, `docs/lancet_supplement.html` and
`docs/lancet_corpus.html`.
...
```sh
LANCET_PLATE_CHECK=off     python lancet_americas/pipeline/08d_figure_dataflow.py   # default: no check
LANCET_PLATE_CHECK=report  python lancet_americas/pipeline/14_extra_figures_rem.py  # accumulates and summarises at the end
LANCET_PLATE_CHECK=strict  python lancet_americas/pipeline/run_all.py               # fails at the first bad plate
```
