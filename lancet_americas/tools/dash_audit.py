...
`common.count_dash_ranges`, que es la misma función con la que se escriben las tablas y las láminas.
    ...
    python3 lancet_americas/tools/dash_audit.py --tables
    python3 lancet_americas/tools/dash_audit.py --plate 13 --variants con_rett --langs en --check strict
    python3 lancet_americas/tools/dash_audit.py --docx lancet_americas/manuscript/manuscript_con_rett_en.docx
"""
...
def audit_plate_module(module: str, variants=None, langs=None, check=None, out_root=None) -> dict:
    ...
    if check:
        os.environ["LANCET_PLATE_CHECK"] = check
    import config as CFG
    if out_root:
