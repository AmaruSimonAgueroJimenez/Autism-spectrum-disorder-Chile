...
Uso:
    python3 lancet_americas/pipeline/13_extra_figures_hospital.py
    python3 lancet_americas/pipeline/13_extra_figures_hospital.py --variants con_rett --langs es --only EF1 EF9
"""
...
if str(LANCET_DIR) not in sys.path:
...
SCRIPT = "lancet_americas/pipeline/13_extra_figures_hospital.py"
...
# repositorio (`lancet_americas/labels.py`), que declara para cada categoría de tres caracteres su glosa
...
def localise_ticks(fig, lang: str) -> None:
    ...
    for ax in fig.axes:
        ...
        for axis, scale, lim in ((ax.xaxis, ax.get_xscale(), ax.get_xlim()),
            ...
            # deja como está.
            if not getattr(axis, "_lancet_localised", False):
