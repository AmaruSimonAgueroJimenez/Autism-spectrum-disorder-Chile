...
Produces `lancet_americas/data_provenance.csv` (one row per artefact, SHA-256 streamed in 1 MiB blocks with
...
`scripts/downloads/rem_pathway_codes.csv` and `lancet_americas/config.py`; nothing is invented. They are written HERE in
...
Run from the repository root:  python3 lancet_americas/pipeline/00_provenance.py [--use-cache] [--limit N]
"""
...
HERE = Path(__file__).resolve().parents[1]  # lancet_americas/
