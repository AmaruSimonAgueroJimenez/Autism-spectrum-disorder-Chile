...
Para cada variante (`con_rett`, `sin_rett`) e idioma (`en`, `es`) construye con `docx_builder.build_document`:
  ...
  manuscript/manuscript_<variante>_<idioma>.docx
      ...
      de `equations_lancet.py`, las láminas suplementarias agrupadas por tema y las tablas suplementarias, todo con
...
Uso:
    python lancet_americas/pipeline/10_manuscript.py                    # todo (4 manuscritos + 4 apéndices + 4 artículos, PDF, miniaturas, memo)
    python lancet_americas/pipeline/10_manuscript.py --langs en --variants con_rett --skip-pdf
    python lancet_americas/pipeline/10_manuscript.py --supp-dpi 0       # sin remuestrear (archivos de ~135 MB)
"""
...
import equations_lancet as EQ  # noqa: E402  (las 33 ecuaciones de la metodología extendida)
...
W = {  # rótulos bilingües del módulo
    ...
    "built": {"en": ("Working version built on {date} by lancet_americas/pipeline/10_manuscript.py from "
              ...
              "es": ("Versión de trabajo construida el {date} por lancet_americas/pipeline/10_manuscript.py a partir de "
...
#: normalised to lower-case alphanumerics. Only the journals of references_lancet.bib that the submission or its
...
def write_memo(report: dict, path: Path = MEMO_PATH) -> Path:
    ...
    lines = []
    A = lines.append
    A("# Memo ejecutivo — Reconocimiento administrativo del autismo en Chile, 2019–2025 (The Lancet Regional Health – Americas)")
    A("")
    A(f"**Fecha:** {today}. **Módulo:** `lancet_americas/pipeline/10_manuscript.py`. Todas las cifras y todos los "
    ...
    A("")
    A("La carpeta `lancet_americas/` contiene el pipeline reproducible (pasos 00–17), el plan de análisis, la procedencia con "
    ...
    A(f"Estructura del archivo del manuscrito. **Artículo:** portada (autores y afiliación de `paper/prose.py`, título corto, "
      ...
      f"S1–S5, la metodología extendida (módulo 12) con las {n_eq} ecuaciones numeradas de `equations_lancet.py` citadas en el "
    ...
    A(f"Legibilidad dentro del panel (fase 4d). La norma fijaba el lienzo; lo que fallaba era la ejecución dentro de los "
      ...
      f"`LANCET_PLATE_CHECK=off|report|strict`; el décimo módulo que dibuja láminas (`06_models.py`) se comprobó con el mismo "
    ...
    A(f"Los totales de control se declaran SIEMPRE con su ámbito, porque los dos ámbitos cuentan cosas distintas y no se suman. "
      ...
      f"del protocolo. La fuente única de los dos totales es `lancet_americas/controls_registry.py`; el consolidado los separa con su "
    ...
    A("- Las láminas usan letras de panel en MINÚSCULA y entre paréntesis, «(a)…(f)», en la figura y en la leyenda, en los dos "
      ...
      "separadores de miles de The Lancet no se aplican.")
...
def main(argv=None) -> int:
    ...
    profile = Path(tempfile.gettempdir()) / "lancet_americas_lo_profile"
