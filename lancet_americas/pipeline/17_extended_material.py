...
Recorre las dos variantes (`con_rett`, `sin_rett`) y los dos idiomas (`es`, `en`) y comprueba:
  ...
  * ecuaciones: las de `equations_lancet.py` están compuestas, se emiten todas una sola vez y se citan;
...
Uso:
    python3 lancet_americas/pipeline/17_extended_material.py               # verifica y escribe índice y controles
    python3 lancet_americas/pipeline/17_extended_material.py --build       # además reconstruye los 12 documentos
    python3 lancet_americas/pipeline/17_extended_material.py --build --supp-dpi 300
    python3 lancet_americas/pipeline/17_extended_material.py --build --skip-pdf
...
TIDY_RE = re.compile(r"(?:lancet_americas/)?outputs/tidy/([A-Za-z0-9_]+\.csv)")
...
def write_index(meta: dict, stats: dict) -> Path:
    ...
    lines = [
        ...
        "",
        f"Generado por `lancet_americas/pipeline/{MODULE}.py` el "
        ...
        f"`LANCET_PLATE_CHECK`: cero defectos en las {n_plate_builds} construcciones ({n_plates} láminas × "
