#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""build_journal.py — the journal-format builder of the sin_rett submission (The Lancet Regional Health – Americas).

Builds, from the journal modules of this folder and the existing corpus builders, into manuscript/04_submission_lancet_americas/:
...
HERE = Path(__file__).resolve().parent            # lancet_americas/journal
...
W = {   # bilingual labels of this module (title page and supplement front matter)
    ...
    "built": {"en": "Built on {date} by lancet_americas/journal/build_journal.py from outputs/values_{variant}.json.",
              "es": "Construido el {date} por lancet_americas/journal/build_journal.py a partir de "
    ...
    for kind, payload in blocks:
        ...
        if payload.get("label", "").split()[-1].isdigit():
            if path.parent != plate_dir:
                warnings.append(f"{payload['label']}: read from {path.parent.relative_to(CFG.OUT) if path.is_relative_to(CFG.OUT) else path.parent}, not from the journal plate folder (LANCET_PLATE_JOURNAL plates not rendered yet)")
...
def render_plates() -> dict:
    ...
    if not hasattr(common, "plate_journal_mode"):
        raise SystemExit("common.py has no journal plate mode yet (LANCET_PLATE_JOURNAL); cannot render the journal plates")
    before = _hash_tree(corpus_paths())
    env = dict(os.environ, LANCET_PLATE_JOURNAL="1", LANCET_PLATE_CHECK="strict")
    runs = []
...
def main(argv=None) -> int:
    ...
    profile = Path(tempfile.gettempdir()) / "lancet_americas_lo_profile_journal"
    for lang in langs:
