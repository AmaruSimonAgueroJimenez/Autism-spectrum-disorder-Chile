# -*- coding: utf-8 -*-
"""journal_config.py — shared configuration of the journal submission (sin_rett, The Lancet Regional Health – Americas).
...
def journal_plate(key: str, lang: str, corpus_png: Path) -> Path:
    ...
    if not png.is_file():
        raise FileNotFoundError(f"journal plate missing for {key} ({lang}): {png} (render with LANCET_PLATE_JOURNAL=1); "
